...
const Comments = () => {
  ...
  const fetchComments = async () => {
    try {
      const {data} = await axios.get('https://blog-backend.cloco.com.au/api/admin/comments')
    ...
    <div className='flex-1 pt-5 px-5 sm:pl-16 bg-blue-50/50'>
      <div className='flex justify-between items-center max-w-3xl'>
        ...
        <div className='flex gap-4'>
          ...
          <button onClick={()=> setFilter('NotApproved')} className={`shadow-custom-sm border rounded-full px-4 py-1 cursor-pointer text-xs ${filter === 'Not Approved' ? "text-primary" : 'text-gray-700'}`}>Not Approved</button>
        </div>
      </div>  

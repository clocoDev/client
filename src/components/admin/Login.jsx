...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
       e.preventDefault();
       try {
        const {data} = await axios.post('https://blog-backend.cloco.com.au/api/admin/login', {email, password})
        ...
        <div className='w-full max-w-sm p-6 max-md:m-6 border border-primary/30 shadow-xl shadow-primary/15 rounded-lg'>
            <div className='flex flex-col items-center justify-center'>
                <div className='w-full py-6 text-center'>
                    ...
                    <p className='font-light'>Enter your credntiald to access the admin panel</p>
                </div>
                <form onSubmit={handleSubmit} className='mt-6 w-full sm:max-w-md text-gray-600'>
                    ...
                    text-white bg-primary rounded cursor-pointer hover:bg-primary/90 transition-all'>Login</button>
                </form>
            </div>
        </div>
    </div>

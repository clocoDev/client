...
const BlogTableItem = ({blog, fetchBlogs,index}) => {
    ...
    const deleteBlog = async ()=>{
        ...
        if(!confirm) return;
        try {
            const {data} = await axios.post('https://blog-backend.cloco.com.au/api/blog/delete', {id: blog._id})
            if(data.success){
    ...
    const togglePublish  = async () => {
        try {
            const {data} = await axios.post('https://blog-backend.cloco.com.au/api/blog/toggle-publish', {id: blog._id})
            if(data.success){
                toast.success(data.message)
                await fetchBlogs()
            } else {
